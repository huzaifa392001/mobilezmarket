...
const Page = () => {
    ...
    const onSubmit = async (values) => {
        let isUserFromSocial = JSON.parse(localStorage.getItem("@user"));
        let payload = {
            ...values,
            phone_number: JSON.parse(localStorage.getItem("@phone")),
        }
        let apiRoute = "/mobile-verification";

        if (isUserFromSocial?.social_login) {
            apiRoute = "/verify";
        }

        try {
            setLoading(true);
            let res = await api.post(apiRoute, payload);
            if (res?.data?.status) {
                getUserData();
            }
        } catch (error) {
